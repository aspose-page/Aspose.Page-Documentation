...
We split one code snippet on several pieces of code: the beginning, the end and each transformation separately. A transformation in PostScript is made allways in graphics state
...
including nested graphics state, and make any transformations or clipping. These transformations will not affect on outer graphics states, but will do on nested one's.
If we make a transformation without **"WriteGraphicsSave()"** and **"WriteGraphicsRestore()"** methods we make it in graphics state of upper level and all content that is in PsDocument will be undergone by 
this transformation.
<br>
<br>
...
4. Save a graphics state. So we created new graphics state, previous graphics state was put in a graphics states stack.
...
7. Restore the graphics state to return back to previous one where applied transformation(s) doesn't affect. In our case it is upper level graphics state.

In this piece of code we create PsDocument from an outpustream and PsSaveOptions, translate upper level graphics state to point 100,100 in order to offset the first rectangle
...
Now we create new graphics state that will be translated on 250 points by X-axis relatively to upper level graphics state and add to this new graphics state the same rectangle path painted with **Blue** color.
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
```
Finally we close current page and save the document.
```C#
    //Close current page
    document.ClosePage();
